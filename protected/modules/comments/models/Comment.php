...
class Comment extends CActiveRecord {
    ...
    public function search() {
        ...
        $criteria->order = 't.status, create_time';
    ...
    public function getPageUrl()
    {
        ...
        if(isset($config['pageUrl']) === true && is_array($config['pageUrl']) === true)
        {
            ...
            return Yii::app()->createUrl($config['pageUrl']['route'].$routeData['id'])."#comment-$this->comment_id";
        }
        return null;
    }
    ...
    public function beforeSave() {
        ...
        if($this->isNewRecord === true && $this->evaluateExpression($this->config['isSuperuser']) === true)
            $this->status = self::STATUS_APPROWED;
        if(!Yii::app()->user->isGuest && Yii::app()->user->type == 'admin' && $this->isNewRecord)
